...
const getCounts = () => {
  const counts = {
    ...
    'full-time': portfolioItems.filter(item => !item.isSideProject && !item.isFreelance).length,
    'contract': portfolioItems.filter(item => !item.isSideProject && item.isFreelance).length,
          ...
          <div className="gap-6 grid grid-cols-1 xl:grid-cols-2">
            {filteredItems.length > 0 ? (
              ...
              <div className="col-span-full py-16 text-center">
                <p className="text-xl text-gray-400">No projects found matching the selected filter.</p>
                <button 
